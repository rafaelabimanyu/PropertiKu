...
<x-dashboard-layout :role="'buyer'" :menuItems="$menuItems" :title="'Welcome back, ' . Auth::user()->name . '!'" :subtitle="'Here\'s what\'s happening with your property search today.'">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <div class="relative overflow-hidden bg-white dark:bg-gray-900 p-7 rounded-3xl shadow-sm border border-slate-100 dark:border-gray-800 group hover:shadow-xl hover:-translate-y-1 transition-all duration-500">
            ...
            <h4 class="text-3xl font-black dark:text-white">{{ $favCount ?? 0 }}</h4>
        </div>
        <div class="relative overflow-hidden bg-white dark:bg-gray-900 p-7 rounded-3xl shadow-sm border border-slate-100 dark:border-gray-800 group hover:shadow-xl hover:-translate-y-1 transition-all duration-500">
            ...
            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Messages</p>
                ...
                @forelse($recentProperties ?? [] as $prop)
                    <div class="flex items-center p-5 bg-white dark:bg-gray-900 rounded-2xl border border-slate-100 dark:border-gray-800 shadow-sm group hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
                        <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-100 to-indigo-50 dark:from-indigo-900/30 dark:to-indigo-800/20 overflow-hidden mr-5 flex-shrink-0 flex items-center justify-center">
                            @if($prop->image)
                                <img src="{{ asset('storage/' . $prop->image) }}" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="flex-1 min-w-0">
                            <h6 class="font-bold dark:text-white mb-1 group-hover:text-indigo-600 transition truncate">{{ $prop->title }}</h6>
                            ...
                            <div class="flex items-center space-x-3">
                                <span class="text-xs font-black text-indigo-600">${{ number_format($prop->price) }}</span>
                                <span class="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded-full text-[8px] font-black uppercase">{{ $prop->status }}</span>
                            </div>
                        </div>
                        <a href="{{ route('properties.show', $prop) }}" class="p-3 bg-slate-50 dark:bg-gray-800 rounded-xl hover:bg-indigo-600 hover:text-white text-slate-400 transition ml-3 flex-shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        </a>
                    </div>
                @empty
                    <div class="text-center py-8 text-slate-500">No properties available.</div>
                @endforelse
